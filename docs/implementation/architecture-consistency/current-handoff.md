...
CURRENT_STAGE=WI0004_APPROVED_PENDING_ATOMIC_SPEC_MERGE
CURRENT_STAGE_STATUS=USER_DECISION_APPROVED
LAST_COMPLETED_STAGE=USER_DECISION_APPROVAL_PASS
CURRENT_BLOCKER=NONE_PENDING_ATOMIC_SPEC_MERGE_PRECHECK
REMOTE_HEAD_BASELINE=82d1fb85f0cc94d84e6b73d267698b337d975c1d
AUTHORITY_BASELINE_COMMIT=82d1fb85f0cc94d84e6b73d267698b337d975c1d
VALIDATION_PROJECT=D:\code\temp\SpecForge-P0-Validation
CURRENT_WI=WI-0004
AUTHORITATIVE_WI_STATE=approved
...
LATEST_STATE_TRANSITION_EVENT_ID=019fe4d4-26c3-7356-bc6a-80a420dbdf61
...
READ_ONLY_RECONCILIATION_RESULT=PASS_WAL_CHECKPOINT_ATTEMPT0003_AND_EXACT_REPO_BINDING
...
OPERATION_BOUNDARY=WI0004_ATOMIC_SPEC_MERGE_ONLY_AFTER_PRECHECK_NO_CODE_PERMISSION_OR_LATER_STAGE
FORBIDDEN_ACTIONS=CANDIDATE_GATE_RERUN,USER_DECISION_RERUN,CODE_PERMISSION,IMPLEMENTATION,VERIFICATION,CLOSE,DAEMON_LIFECYCLE,OPENCODE_LIFECYCLE
NEXT_STAGE=WI0004_ATOMIC_SPEC_MERGE
NEXT_LEGAL_ACTION=REVALIDATE_APPROVAL_AND_FROZEN_CANDIDATE_THEN_RUN_ONE_ATOMIC_SPEC_MERGE
STOP_CONDITION=ATOMIC_SPEC_MERGE_COMPLETE_OR_FAIL_CLOSED
...
LATEST_DEVELOPMENT_FAILURE=ERR-315_V165_VALIDATOR_NEWLINE_ESCAPE_FALSE_NEGATIVE_CLOSED_PREDELIVERY
...
PRIOR_FAILURE_RECONCILIATION=PASS_ERR306_ERR315_BACKFILLED_AND_V163_USER_DECISION_VALIDATED
UNRECORDED_FAILURES=0
EXPERIENCE_FILE_READ=YES_LATEST_WITH_ERR306_ERR315
EXPERIENCE_RE_READ_REQUIRED=NO_AFTER_ERR306_ERR315_BACKFILL
```
- V152 已完成 V133 D15-D19 最终 Authority 内容的 commit + push；本 handoff 不再把已完成的 V152 remote sync 写成当前 blocker。
- committed handoff 不保存“自身提交后的当前 Git HEAD”这种自引用事实；最新仓库 HEAD 必须由 structured live Git ref / receipt 取得，`REMOTE_HEAD_BASELINE` 只保存进入本次 Stage 前的基线。
- V162 已对 `D:\code\temp\SpecForge-P0-Validation` 完成 exact repository binding、WAL/checkpoint、attempt-0003 与 Candidate/Gate 原始证据只读对账；V163 已用用户明确原话“同意继续”记录正式 User Decision。
- WI-0004 权威状态已由 StateManager 从 `approval_required` 推进到 `approved`；Candidate、Gate、Project Spec 在 User Decision 期间保持不变。
- 下一阶段只允许执行一次 Atomic Spec Merge：必须先重新验证 live Authority ref、Validation repo HEAD、`approved` 状态、user_decision hash、Candidate/Gate frozen hash；任何漂移 Fail Closed。
- Atomic Spec Merge 成功后立即停止并审计 merge report、Project Spec version、状态和写入范围；本轮不得顺带执行 Code Permission、Implementation、Verification 或 Close。
- daemon / OpenCode 生命周期仍由用户手工控制；自动包只允许使用已经运行的 daemon，不得启动、停止或重启。
