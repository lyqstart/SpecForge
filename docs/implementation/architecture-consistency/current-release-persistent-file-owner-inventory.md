...
| Changed Files Audit | `.specforge/work-items/<WI>/changed_files_audit.md` | `sf_changed_files_audit` owns normal/no-code audit; Close Gate may re-audit after operation normalization; all governance consumers use the shared verdict authority | exact Markdown contract `changed-files-audit/v1`; Work Item identity, canonical PASS/FAIL and required safety counts | shared current parser in Daemon Core; no legacy format inference or migration transition | `CONFIRMED / ENABLED`; every current producer emits the same contract, every contextual consumer binds the selected Work Item, and producer persistence failure creates a HardStop and fails closed |
| Merge Report | `.specforge/work-items/<WI>/merge_report.md` | Merge Runner transaction only; lifecycle does not initialize it and generic Artifact Writer rejects direct or disguised writes | `@specforge/types/merge-report-contract`, `merge-report/v1`, bound to Work Item identity, status and merge counts | current Markdown contract only; no legacy JSON compatibility or inferred migration transition | `CONFIRMED`; producer and all active consumers exact-validated, ERR-1490 closed |
| Diagnostic observability store | `.specforge/logs/observability/userlevel/**`; `.specforge/logs/observability/daemon/**` | installed Thin client recorder owns `userlevel`; Daemon recorder owns `daemon` | record `schema_version=1.4`; each owner has its own category streams, `index.jsonl`, `payloads/by-sha256` and `errors/by-sha256` | non-authoritative diagnostic evidence; not a Runtime state descriptor | `CONFIRMED / ENABLED`; physical write roots are disjoint and Runtime WAL remains authoritative |
...
| User-level install transaction | `release/release-manifest.json` → `<OpenCode config>/specforge-manifest.json`; OpenCode assets at config root and private SpecForge assets under `sf-user/**` | `SHARED_COMPONENT_REGISTRY` enumerates source assets; release producer binds source hash/size; installer consumes only a verified release install set and writes the installed manifest | release and installed manifests use file-specific schema `1.0`; every managed file is hash/size bound | transaction connected through hash-bound release manifest; current 108-file isolated install/upgrade/rollback verified | `SOURCE_CONVERGED / EXECUTION_VALIDATION_PENDING`; layout source changed at `d602874d`, no real user-level redeploy evidence in this recovery session |
| User-level Daemon handshake | `<OpenCode config>/sf-user/runtime/handshake.json` | Daemon `HandshakeManager` is the only writer; CLI, installed Tool thin client, installed plugin client and service-management readers consume the canonical path contract | root `schema_version=1.0`; current local writer emits `bound_to=127.0.0.1`, ownership identity and task artifact contract version; service-management uses one shared runtime parser | transient runtime discovery/authentication authority, regenerated by the current Daemon rather than migrated as durable state | `SOURCE_CONVERGED / EXECUTION_VALIDATION_PENDING`; source closure at `fedabb7b` + comment correction `ee40d40f`, no CI or real user-level redeploy evidence yet |
...
treat this current transaction authority as a generated file. The journal remains a transient installer recovery surface under `sf-user/` and is not part of the immutable managed install set.
...
### Runtime state and event log

SPS-1.0 is authoritative. `StateManager.initialize()` now
...
### Diagnostic observability — resolved

The current release separates authoritative workflow state from diagnostic
evidence. `StateManager`/WAL alone own `.specforge/runtime/events.jsonl`.
The installed Thin client writes only
`.specforge/logs/observability/userlevel/**`; the Daemon recorder writes only
`.specforge/logs/observability/daemon/**`. Each source owns its own category
streams, index, payload-address and error-address subtrees, so no physical
append target has two producers. Diagnostic failures remain best-effort and
cannot advance or rewrite Runtime state.

Production import reconstruction proved that the former package-local
EventLogger, CAS, Event Bus, QueryAPI, AnalystEngine, ModeSwitch, North Star
runtime and SfAnalyst implementation had no current production caller. They
and their implementation-only tests exited the current build instead of being
retained for legacy compatibility. `@specforge/observability` now owns only
shared event types and the strict three-tier policy contract. The `sf-analyst`
Agent role remains a consumer of governed diagnostic evidence; it is not a
second persistent store owner.
...
### User-level installer transaction — source-converged
...
### Observability owner — final current closure

```text
OWNER_FAMILY=DIAGNOSTIC_OBSERVABILITY
AUTHORITATIVE_STATE_OWNER=DAEMON_STATEMANAGER_WAL_AT_.specforge/runtime/events.jsonl
USERLEVEL_DIAGNOSTIC_OWNER=.specforge/logs/observability/userlevel/**
DAEMON_DIAGNOSTIC_OWNER=.specforge/logs/observability/daemon/**
PHYSICAL_OWNER_OVERLAP=NONE
OBSERVABILITY_PACKAGE_ROLE=EVENT_TYPES_AND_POLICY_CONTRACT_ONLY
REMOVED_UNCALLED_RUNTIME=EVENT_LOGGER;PACKAGE_CAS;PACKAGE_EVENT_BUS;MODE_SWITCH;QUERY_API;ANALYST_ENGINE;NORTH_STAR_RUNTIME;SF_ANALYST_CLASS
TARGET_REGRESSION=OBSERVABILITY_47_PASS;DAEMON_OWNER_AND_CONSUMERS_24_PASS;INSTALLER_3_PASS;LOCK_7_PASS
ROOT_BUILD=PASS_16_WORKSPACES
ROOT_REGRESSION=PASS_16_WORKSPACES_EXIT_0
ERR1013_PARENT_STATUS=CLOSED
OPEN_ERRORS=NONE
NEXT_LEGAL_ACTION=FINAL_DIFF_STATUS_AUDIT_AND_LOCAL_CHECKPOINT_COMMIT
```
