...
| Remaining governance evidence | `.specforge/work-items/<WI>/merge_report.md` | V6 assigns exclusive authority to Merge Runner, while actual writer/consumer closure still requires consolidation | V6 design says `merge_report.json`; complete current implementation uses Markdown | contract conflict open as ERR-1490; descriptor decision deferred until format authority is resolved | `CONTRACT_CONFLICT`; do not modify either format path until authority resolution is recorded |
