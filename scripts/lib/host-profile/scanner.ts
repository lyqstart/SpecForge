...
export function getHostProfilePath(): string {
  return path.join(os.homedir(), '.specforge', 'host-profile.json');
}
...
export async function scanHostProfile(opts: ScanOptions = {}): Promise<ScanResult> {
  ...
  const specforge: SpecForgePaths = {
    install_root: path.join(os.homedir(), '.specforge'),
    logs_dir: path.join(os.homedir(), '.specforge', 'logs'),
  };
 ...
 * 读取缓存的 profile
 */
async function loadCachedProfile(profilePath: string): Promise<HostProfile | null> {
 ...
 */
function isCacheFresh(profile: HostProfile): boolean {
  // hostname 必须一致
