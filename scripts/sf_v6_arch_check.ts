...
用法: bun run scripts/sf_v6_arch_check.ts [work_item_id] [options]
...
示例:
  ...
  bun run scripts/sf_v6_arch_check.ts --json
  bun run scripts/sf_v6_arch_check.ts --strict
...
// 仅在直接运行时执行（bun run / node 直接调用本文件）
// 历史教训：早先此文件曾放在 .opencode/tools/ 下，OpenCode 启动会扫描该目录
// 把所有 .ts import 注册为 tool，导致顶层 main() 在 import 时立刻执行 + process.exit
// 把 OpenCode 自身进程杀掉。现在已搬到 scripts/（按 project-structure.md 规则 1 应放此处），
// 但仍保留这个守卫，防止有人 import 它做单元测试/逻辑复用时出现意外副作用。
if (import.meta.main) {
  main();
}