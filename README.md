...
```bash
# 安装到目标项目
bun scripts/sf-installer.ts install --target D:\your\project

# 如果目标项目已有旧版 SpecForge（无 manifest），用 --force
bun scripts/sf-installer.ts install --target D:\your\project --force
```
...
```bash
# 查看已安装版本
bun scripts/sf-installer.ts --version --target D:\your\project

# 校验安装完整性
bun scripts/sf-installer.ts verify --target D:\your\project

# 升级到新版本（仅更新变化的文件，保留运行时数据）
bun scripts/sf-installer.ts upgrade --target D:\your\project

# 卸载（保留运行时数据：specs/、sessions/、archive/）
bun scripts/sf-installer.ts uninstall --target D:\your\project

# 完全卸载（删除所有 SpecForge 文件和运行时数据）
bun scripts/sf-installer.ts uninstall --target D:\your\project --purge
```
...
- 安装器只操作 `sf-`/`sf_` 前缀的文件，不会覆盖你的代码
- `opencode.json` 合并时只添加 `sf-*` agent 定义，保留你现有的配置
- `package.json` 合并时只添加 SpecForge 所需的 devDependencies
- 升级时保留所有运行时数据（state.json、specs/、sessions/、archive/）
- 通过 `specforge/manifest.json` 追踪已部署文件和版本
...
# Bash
bash scripts/install.sh D:/your/project
```

> 注意：旧脚本不生成 manifest.json，无法使用 verify/upgrade/uninstall 命令。建议迁移到统一安装器。

### 安装后

```bash
cd <你的项目>
bun install          # 安装依赖（如果安装器未自动执行）
