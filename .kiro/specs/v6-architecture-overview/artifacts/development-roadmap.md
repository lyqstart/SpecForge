...
**目标**：启动跨模块集成联调，验证端到端场景。

> **变更说明（2026-05-18）**：`self-healing` 模块延期至 V6.1 开发。V6.0 仅实现诊断阶段（Diagnose），该功能为增强能力而非核心依赖，不影响系统部署运行。Property 24/25 的 PBT 验证同步延期至 V6.1。

| 工作包 | 范围 | 依赖 | 承接 Property |
|---|---|---|---|
...
**Checkpoint W5（发版判据）**：
- 6 条门槛全部通过
- 28 条 Correctness Property 的 PBT 全部绿（Property 24/25 延期至 V6.1）
- 打 V6.0 stable tag
...
- Wave 3：集成测试（self-healing 延期至 V6.1）
- Wave 4–5：分发 + 验证，不按任务数计
...
| 日期 | 变更 |
|---|---|
| 2026-05-18 | self-healing 延期至 V6.1：从 W3 移除，W3 简化为纯集成测试；W5 发版判据从 30 条 Property 调整为 28 条（Property 24/25 随 self-healing 延期）|
