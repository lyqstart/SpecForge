...
| sf-verifier | subagent | 验证与测试执行（只读），V3.7 起生成结构化 JSON 验证报告 | anthropic/claude-sonnet-4-20250514 | task=deny, edit=deny, bash=ask |
...
| 文件路径 | 用途 |
|----------|------|
| `.opencode/tools/lib/sf_gate_types.ts` | 共享 GateResult / SyncSummary 接口（从 sf_requirements_gate_core.ts 迁移） |
| `.opencode/tools/lib/sf_verification_types.ts` | V3.7 类型定义（VerificationType、ParsedTaskVerification、VerificationReport 等） |
| `.opencode/tools/lib/sf_markdown_verification_parser.ts` | tasks.md 验证字段解析器（typed/legacy 格式识别） |
| `.opencode/tools/lib/sf_verifier_execution_core.ts` | sf-verifier 执行核心（collect-all 策略、原子写入） |
...
## 15. V3.7 验证策略（Verification Strategy）

### 15.1 概述

V3.7 为 SpecForge 引入系统化的验证策略管理能力，在 requirements → design → tasks 三个层面建立类型化的验证绑定链路：

- **requirements.md 层**：每个需求可声明可选的 `verification_strategy` 字段，指定应使用哪种检测方式（unit / property / integration / e2e / regression）验证该需求
- **design.md 层**：Correctness Properties 新增 `test_type`、`test_file`、`requirement_ref` 字段，将设计层面的验证意图与测试文件明确关联
- **tasks.md 层**：`verification_commands` 从平铺字符串列表升级为按类型分组的结构化对象；新增 `refs` 字段建立机器可追溯链路；新增 `manual_verification_checks` 字段分离非可执行检查项
- **sf-verifier 层**：读取类型化命令，使用 collect-all 策略执行，原子写入结构化 `verification_report.json`，同时保留 V3.6 兼容的 `verification_report.md`
- **sf_verification_gate 层**：按 Verification_Type 分别检查测试结果，在 `details.type_results` 中报告每种类型的状态

所有变更向后兼容：`verification_strategy` 为可选字段，不声明时行为与 V3.6 完全一致；旧格式 `verification_commands` 不改变 pass/fail 语义。

### 15.2 新增文件

| 文件 | 用途 |
|------|------|
| `.opencode/tools/lib/sf_gate_types.ts` | 共享 GateResult 接口（从 sf_requirements_gate_core.ts 迁移），含可选 `details?: Record<string, unknown>` 字段 |
| `.opencode/tools/lib/sf_verification_types.ts` | V3.7 类型定义（VerificationType、ParsedTaskVerification、VerificationReport 等） |
| `.opencode/tools/lib/sf_markdown_verification_parser.ts` | tasks.md 验证字段解析器（typed/legacy 格式两层识别） |
| `.opencode/tools/lib/sf_verifier_execution_core.ts` | sf-verifier 执行核心（collect-all 策略、原子写入、双格式报告输出） |

### 15.3 sf-verifier V3.7 执行协议

sf-verifier 在 verification 阶段执行类型化验证命令，遵循以下协议：

- **Stale report cleanup**：执行前删除旧的 `verification_report.json`，确保 sf_verification_gate 不会读取过期报告
- **Collect-all 策略**：当某条命令执行失败（exit_code != 0）时，记录为 `status="failed"` 并继续执行后续命令，不中断整个验证流程
- **双格式输出**：
  - `verification_report.json`（V3.7 结构化报告）— sf_verification_gate 优先读取
  - `verification_report.md`（V3.6 兼容报告）— 向后兼容
- **原子写入**：先写入临时文件（`.verification_report.json.tmp`），完成后 rename 为最终文件名，确保调用方只会看到 `"completed"` 或文件不存在两种状态
- **Type 字段处理**：typed 命令的记录包含 `type` 字段（值为对应的 VerificationType）；旧格式命令的记录省略 `type` 字段
- **manual_verification_checks 不执行**：该字段中的条目不被 sf-verifier 执行，不出现在报告中

### 15.4 verification_report.json Schema

sf-verifier 产出的结构化验证报告满足以下最小 Schema：
...
- `status`：`"completed"` | `"incomplete"`，必填
- `commands`：数组，必填

**commands 元素字段：**
- `type`：VerificationType 合法值之一，可选（typed 命令必填，旧格式命令省略）
- `command`：执行的命令字符串，必填
- `status`：`"passed"` | `"failed"` | `"skipped"`，必填
- `exit_code`：整数，必填（skipped 时为 -1）
- `stdout` / `stderr`：字符串，可选

**sf_verification_gate 对报告完整性的处理：**
- 若 `verification_report.json` 不存在、JSON 格式非法、或 `status` 不为 `"completed"` → fail，blocking_issue: `"Verification report is missing, malformed, or incomplete."`
- 查找优先级：优先读取 `verification_report.json`，不存在时回退到 V3.6 行为

### 15.5 Gate V3.7 扩展

**sf_requirements_gate（默认模式新增检查）：**
- 扫描 requirements.md 中所有需求的 `verification_strategy` 字段
- 验证值合法性：必须是 `unit`、`property`、`integration`、`e2e`、`regression` 的非空子集
- 大小写不敏感解析，规范化为小写后传播
- 空列表 → fail；重复值 → warning + 去重；未声明 → pass（不强制）

**sf_design_gate（默认模式新增检查）：**
- 验证 Correctness Properties 中 `test_type` 字段的值合法性
- 非法 `test_type` 值 → fail
- 仅做本地语法检查，不跨文件读取 requirements.md

**sf_tasks_gate（默认模式新增检查）：**
- 类型键合法性验证：非法类型键（如 `smoke:`）→ fail
- 旧格式 → pass + non-blocking migration warning
- 交叉验证（5 个场景）：
  - 场景 A：typed task 无 REQ-N refs → fail
  - 场景 B：refs 指向的 REQ 无 verification_strategy → 忽略
  - 场景 C：refs 指向多个 REQ → 取 verification_strategy 并集
  - 场景 D：Planned_Verification_Types 未覆盖 Declared_Required_Types → fail
  - 场景 E：typed task 有 property 命令但 refs 中无 CP-N → fail

**sf_verification_gate（默认模式新增检查）：**
- 按 Verification_Type 分别检查测试结果
- 输出 `details.type_results`，每种类型状态为 `"passed"` | `"missing"` | `"failed"` | `"skipped"`
- 支持 `required_types` 可选参数显式指定必须通过的类型集合
- 旧格式 tasks.md → 回退到 V3.6 默认行为
- 混合格式 → typed 部分按类型检查 + non-blocking warning

**sf_doc_lint（新增检查）：**
- 对类型化 `verification_commands` 执行与 sf_tasks_gate 相同的类型键合法性验证
- 旧格式 → pass + warning（与 sf_tasks_gate 行为一致）
- `manual_verification_checks` 字段 → 接受并验证结构（必须是字符串列表）
