...
export class Req25Parser {
   ...
   */
  private extractReq25Section(markdown: string): {
    rawSection: string;
    p0Section: string;
    p1Section: string;
    p2Section: string;
   ...
   */
  private extractScopeLevel(section: string, level: string): string {
  ...
  extractCapabilities(sectionText: string, scopeTag: ScopeTag): CapabilityDefinition[] {
    ...
    for (const itemText of items) {
      ...
      if (/^THE\s+Requirements/i.test(itemText)) continue;
      if (/^Acceptance\s+Criteria/i.test(itemText)) continue;
      if (/^P\d+\s*必做项|^P\d+\s*项/i.test(itemText)) continue;
      if (/^V\d+\.\d+/i.test(itemText)) continue;
      if (/^\d+\.\s*THE/i.test(itemText)) continue;
      if (/^分组为/i.test(itemText)) continue;
   ...
   */
  private createEmptyData(): Req25Data {
    return {
      p0: [],
      p1: [],
      p2: [],
