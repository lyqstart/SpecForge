...
export class AuthFailedError extends CliError {
  ...
  constructor(
    ...
    hint = 'Check your token in <OpenCode config>/sf-user/runtime/handshake.json or restart the Daemon.',
  ) {
    super(message, hint);
  }
}
...
export function toCliError(err: unknown): CliError {
  ...
  if (err instanceof Error) {
    ...
    if (errName === 'DaemonAuthError' || errAny.code === 'AUTH_FAILED') {
      return new AuthFailedError(err.message, 'Check your token in <OpenCode config>/sf-user/runtime/handshake.json or restart the Daemon.');
    }
