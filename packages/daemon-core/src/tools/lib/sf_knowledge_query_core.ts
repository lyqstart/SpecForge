...
export async function getOverview(baseDir: string): Promise<OverviewResult> {
  ...
  if (error) {
    return {
      query_type: "get_overview",
      nodes_by_type: { requirement: 0, design_decision: 0, task: 0, code_file: 0, refactor_target: 0, ops_action: 0 },
      edges_by_type: { traces_to: 0, decomposes_to: 0, modifies: 0, implements: 0, affects: 0 },
      work_items: [],
      total_nodes: 0,
      total_edges: 0,
    }
  ...
  const nodesByType: Record<NodeType, number> = {
    refactor_target: 0, ops_action: 0,
    requirement: 0,
    design_decision: 0,
    task: 0,
    code_file: 0,
