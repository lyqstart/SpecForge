/** Workflow-to-gate mapping and phase-aware strictness lookup. */
...
type WorkflowPath =
  ...
  | 'task_change_path' | 'code_only_fast_path' | 'spec_migration_path'
  | 'contract_change_path' | 'rollback_path';

function dedupe(items: GateIdV11[]): GateIdV11[] { return Array.from(new Set(items)); }
...
const investigationCandidateGates: GateIdV11[] = [
  ...commonCandidateGates, 'required_files_gate', 'candidate_manifest_gate', 'path_policy_gate', 'workflow_specific_gate',
];
...
function getCandidateGates(workflowPath: string, candidatePhase: CandidateGatePhaseV11, workflowType?: string): GateIdV11[] {
  ...
  if (workflowType === 'contract_change' || workflowPath === 'contract_change_path') return contractCandidateGates;
  switch (workflowPath as WorkflowPath) {
    ...
    case 'spec_migration_path':
      return candidatePhase === 'design' || candidatePhase === 'requirements'
        ? designCandidateGates
        : fullSpecCandidateGates;
    case 'task_change_path':
      return [...commonCandidateGates, 'required_files_gate', 'candidate_manifest_gate', 'path_policy_gate', 'contract_integrity_gate', 'trace_gate', 'workflow_specific_gate'];
    case 'code_only_fast_path':
      // Fast Path skips upper-spec modification, never upper-spec consistency.
      return [...commonCandidateGates, 'path_policy_gate', 'candidate_manifest_gate', 'spec_consistency_gate', 'contract_integrity_gate', 'trace_gate'];
    case 'rollback_path':
...
function getLegacyAllGates(workflowPath: string, workflowType?: string): GateIdV11[] {
  if (workflowType === 'investigation') return [...investigationCandidateGates, 'merge_ready_gate', 'post_merge_gate', 'verification_gate', 'close_gate'];
  if (workflowType === 'contract_change' || workflowPath === 'contract_change_path') return [...contractCandidateGates, 'merge_ready_gate', 'post_merge_gate', 'verification_gate', 'close_gate'];
  switch (workflowPath as WorkflowPath) {
    case 'requirement_change_path':
    case 'design_change_path':
    case 'architecture_change_path':
