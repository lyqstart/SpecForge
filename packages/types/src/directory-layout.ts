...
export const LAYOUT = {
  ...
  workItemFiles: {
    ...
    /** `<root>/.specforge/work-items/<WI-ID>/requirements.md`（legacy 兼容读取） */
    requirements: "requirements.md",
    /** `<root>/.specforge/work-items/<WI-ID>/design.md`（legacy 兼容读取） */
    design: "design.md",
    ...
    /** Candidate 子树中的权威规格产物布局。 */
    candidateFiles: {
      /** `<root>/.specforge/work-items/<WI-ID>/candidates/project/` */
      project: "project",
      /** `<root>/.specforge/work-items/<WI-ID>/candidates/project/modules/` */
      modulesRoot: "project/modules",
      /** 模块级 requirements Candidate 文件名。 */
      requirements: "requirements.candidate.md",
      /** 模块级 design Candidate 文件名。 */
      design: "design.candidate.md",
      /** `<root>/.specforge/work-items/<WI-ID>/candidates/tasks.md` */
      tasks: "tasks.md",
      /** `<root>/.specforge/work-items/<WI-ID>/candidates/trace_delta.md` */
      traceDelta: "trace_delta.md",
    },
...
export function projectSpecManifest(projectRoot: string): string {
  return path.join(
    projectRoot,
    SPEC_DIR_NAME,
    LAYOUT.projectFiles.specManifest,
  );
}
...
export function projectExtensionRegistry(projectRoot: string): string {
  return path.join(
    projectRoot,
    SPEC_DIR_NAME,
    LAYOUT.projectFiles.extensionRegistry,
  );
}
...
export function projectRequirementsIndex(projectRoot: string): string {
  return path.join(
    projectRoot,
    SPEC_DIR_NAME,
    LAYOUT.projectFiles.requirementsIndex,
  );
}
...
export function projectArchitecture(projectRoot: string): string {
  return path.join(
    projectRoot,
    SPEC_DIR_NAME,
    LAYOUT.projectFiles.architecture,
  );
}
...
export function workItemJson(projectRoot: string, workItemId: string): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.workItemJson,
  );
}
 ...
 * intake.md 路径。
 */
export function workItemIntake(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.intake,
  );
}

/** change_classification.md 路径。 */
export function workItemChangeClassification(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.changeClassification,
  );
}

/** impact_analysis.md 路径。 */
export function workItemImpactAnalysis(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.impactAnalysis,
  );
}

/** trigger_result.json 路径。 */
export function workItemTriggerResult(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.triggerResult,
  );
}

/** Work Item 顶层 requirements.md（legacy 兼容读取）路径。 */
export function workItemRequirements(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.requirements,
  );
}

/** Work Item 顶层 design.md（legacy 兼容读取）路径。 */
export function workItemDesign(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.design,
  );
}

/** Work Item 顶层 tasks.md（legacy 兼容读取）路径。 */
export function workItemTasks(projectRoot: string, workItemId: string): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.tasks,
  );
}

/** Work Item 顶层 trace_delta.md（合并后/兼容读取）路径。 */
export function workItemTraceDelta(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.traceDelta,
  );
}
 ...
 */
export function workItemRuntimeLog(
  projectRoot: string,
  workItemId: string,
): string {
 ...
 */
export function workItemCandidateManifest(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.candidateManifest,
  );
}
 ...
 * candidates 目录路径。
 */
export function workItemCandidatesRoot(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.candidates,
  );
}

/** Candidate 项目规格根目录。 */
export function workItemCandidateProjectRoot(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemCandidatesRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.candidateFiles.project,
  );
}

/** Candidate 模块根目录。 */
export function workItemCandidateModulesRoot(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemCandidatesRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.candidateFiles.modulesRoot,
  );
}

/** 单个 Candidate 模块目录。 */
export function workItemCandidateModuleRoot(
  projectRoot: string,
  workItemId: string,
  moduleName: string,
): string {
  return path.join(
    workItemCandidateModulesRoot(projectRoot, workItemId),
    moduleName,
  );
}

/** 模块级 requirements Candidate 的权威路径。 */
export function workItemCandidateRequirements(
  projectRoot: string,
  workItemId: string,
  moduleName: string,
): string {
  return path.join(
    workItemCandidateModuleRoot(projectRoot, workItemId, moduleName),
    LAYOUT.workItemFiles.candidateFiles.requirements,
  );
}

/** 模块级 design Candidate 的权威路径。 */
export function workItemCandidateDesign(
  projectRoot: string,
  workItemId: string,
  moduleName: string,
): string {
  return path.join(
    workItemCandidateModuleRoot(projectRoot, workItemId, moduleName),
    LAYOUT.workItemFiles.candidateFiles.design,
  );
}

/** tasks Candidate 的权威路径。 */
export function workItemCandidateTasks(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemCandidatesRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.candidateFiles.tasks,
  );
}

/** trace_delta Candidate 的权威路径。 */
export function workItemCandidateTraceDelta(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemCandidatesRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.candidateFiles.traceDelta,
  );
}
 ...
 * gates 目录路径。
 */
export function workItemGatesRoot(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.gates,
  );
}
 ...
 * gate_summary.md 路径。
 */
export function workItemGateSummary(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.gateSummary,
  );
}
 ...
 */
export function workItemUserDecision(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.userDecision,
  );
}
 ...
 */
export function workItemVerificationReport(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.verificationReport,
  );
}
 ...
 * merge_report.md 路径。
 */
export function workItemMergeReport(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.mergeReport,
  );
}
 ...
 * evidence 目录路径。
 */
export function workItemEvidenceRoot(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.evidence,
  );
}
 ...
 */
export function workItemEvidenceManifest(
  projectRoot: string,
  workItemId: string,
): string {
  return path.join(
    workItemRoot(projectRoot, workItemId),
    LAYOUT.workItemFiles.evidenceManifest,
  );
}
