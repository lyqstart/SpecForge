...
export const sf_specforge: Plugin = async ({ directory, client }) => {
  // DIAGNOSTIC: Write a file to confirm plugin was loaded by OpenCode
  try {
    const diagDir = join(detectProjectRoot(directory), "specforge", "logs")
    await mkdir(diagDir, { recursive: true })
    await writeFile(join(diagDir, "plugin_loaded.txt"), `Plugin loaded at ${new Date().toISOString()}\ndirectory: ${directory}\n`, "utf-8")
  } catch {}
