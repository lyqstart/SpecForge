...
**执行步骤：**
1. 调用 `sf_state_transition`（from_state=""，to_state="intake"，workflow_type="change_request"）创建新 Work Item
...
4. 调用 `sf_state_transition`（from_state="intake"，to_state="impact_analysis"，evidence="intake.md generated"）
...
**执行步骤：**
1. 调用 `sf_state_read` 确认当前状态为 `impact_analysis`
2. 调用 `sf_context_build`（work_item_id=<id>, phase="requirements"）构建阶段上下文，注入到子 Agent prompt 中
...
4. 等待子 Agent 完成，确认 `.specforge/specs/<work_item_id>/impact_analysis.md` 已生成
5. 调用 `sf_state_transition`（from_state="impact_analysis"，to_state="impact_analysis_gate"，evidence="impact_analysis.md generated"）
...
2. 根据 Gate 结果：
   - **pass** → KG 同步（scope=requirements）→ 调用 `sf_state_transition`（to_state="design_delta"）
   - **fail** → 调用 `sf_state_transition`（to_state="impact_analysis"），重新调度 sf-requirements 修订
   - **blocked** → 调用 `sf_state_transition`（to_state="blocked"），向用户报告阻塞原因
...
**执行步骤：**
1. 调用 `sf_state_read` 确认当前状态为 `design_delta`
...
4. 等待子 Agent 完成，确认 `.specforge/specs/<work_item_id>/design_delta.md` 已生成
5. 调用 `sf_state_transition`（from_state="design_delta"，to_state="design_gate"，evidence="design_delta.md generated"）
...
2. 根据 Gate 结果：
   - **pass** → KG 同步（scope=design）→ 调用 `sf_state_transition`（to_state="tasks"）
   - **fail** → 调用 `sf_state_transition`（to_state="design_delta"），重新调度 sf-design 修订
...
2. 根据 Gate 结果：
   - **pass** → KG 同步（scope=tasks）→ 调用 `sf_state_transition`（to_state="development"）
   - **fail** → 调用 `sf_state_transition`（to_state="tasks"），重新调度 sf-task-planner
   - **blocked** → 调用 `sf_state_transition`（to_state="blocked"）
...
6. 所有 Task 完成后，调用 `sf_state_transition`（from_state="development"，to_state="review"）
