...
Quick Change 只能处理无需求、无设计、无架构、无接口契约变化的小改动。它的目标是轻量，不是降低真实性。

- **依据**：必须有明确证据证明 `unknowns=[]`，且没有用户可见新能力、验收标准变化、数据语义变化或接口契约变化。
- **承接**：tasks 必须承接 intake 的小改动目标，并明确不影响项目级规格。若发现需要新增 REQ / DD / trace / merge entry，必须升级 workflow。
- **验证**：仍需运行能证明实际行为变化的 verification_commands；不能只靠文件改动或编译通过关闭。
- **融合**：candidate_manifest.entries 保持 `[]`；trace_delta 必须标注 no spec impact，并说明依据。
