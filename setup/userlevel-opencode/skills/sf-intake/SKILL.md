...
## B1A 四问模型 intake 约束（依据 / 承接 / 验证 / 融合）

在 B1 收集用户需求后，orchestrator 必须把用户回答整理成四类信息，供后续 requirements/design/task 使用：
...
### 2. 承接：用户目标必须进入后续 requirements

用户表达的核心结果不能在后续 requirements 中被静默遗漏，也不能被写入 out_of_scope。若用户说“保存到服务器”，后续必须至少形成“服务器保存”的需求或明确 blocked。

### 3. 验证：intake 阶段要提前识别验收落点

对每个核心用户目标，记录最终可观察结果：UI 可见、文件存在、数据库记录、服务器日志、API 响应、部署状态等。

### 4. 融合：判断本 WI 可能影响项目级真相源的类型

初步判断本 WI 是否可能影响：requirements、design、architecture、decisions、trace、evidence 或仅为代码修复。该判断不是最终结论，但必须传递给后续 workflow。

### 5. 禁止假设

以下内容不得在 intake 后被默认当作事实：
