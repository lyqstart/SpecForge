...
Refactor 的核心目标是改善内部结构且保持外部行为不变。任何行为变化都不是 refactor，必须升级到其他 workflow。

- **依据**：refactor_analysis 必须基于代码观测说明代码问题、重构目标和不变行为边界。
- **承接**：refactor_plan 必须覆盖每个代码问题和不变行为声明；执行步骤必须确保每步后代码可运行。
- **验证**：必须证明所有不变行为仍成立，现有测试通过，并说明代码质量改善证据；不能只证明构建成功。
- **融合**：通常不改业务需求；若改变架构/设计决策，必须更新 project decisions/design，否则声明 evidence_only 或 no_project_spec_change_reason。
