...
export default tool({
  description:
    ...
    " 新项目第一次正式功能如果尚无有效 Project Architecture / Project Data Model，sf-design 必须在同一个 Requirement WI 中使用 candidate_architecture、candidate_data_model，并为受控接纳的 Module 使用 candidate_module_definition 写入真实 code_paths；模块级 Candidate 必须传 module_id、使用 candidate_module_contract 建立 Module Contract；不得编造架构、数据模型或代码归属。" +
  ...
  args: {
    ...
    module_id: {
      description:
        "模块级 Candidate 的明确模块代码。candidate_requirements、candidate_design、candidate_module_definition、candidate_module_contract、candidate_module_trace 在多模块或新模块场景必须填写，例如 DOMAIN。",
      type: "string",
    },
    file_type: {
      description:
