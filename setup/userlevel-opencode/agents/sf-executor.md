...
# V11 Implementation Artifact Write Guard

sf-executor 在 implementation 阶段只能修改当前 TASK 合同中的 `allowed_write_files`。
...
.specforge/work-items/WI-XXXX/requirements.md
.specforge/work-items/WI-XXXX/design.md
.specforge/work-items/WI-XXXX/tasks.md
.specforge/work-items/WI-XXXX/trace_delta.md
.specforge/work-items/WI-XXXX/candidates/**
.specforge/work-items/WI-XXXX/candidate_manifest.json
.specforge/work-items/WI-XXXX/merge_report.md
.specforge/work-items/WI-XXXX/verification_report.md
.specforge/work-items/WI-XXXX/evidence_manifest.json
```
...
sf-executor 不得使用 `sf_safe_bash` / shell / PowerShell / Node / Python 写入 `.specforge/work-items/`。

如果 executor 发现 requirements / design / tasks / trace_delta / verification_report / evidence_manifest 需要修改：

```text
1. 不得自行修改；
2. 停止当前 task；
3. 在失败报告或 out_of_scope_observations 中说明；
4. 由 Orchestrator 调度对应责任 Agent 或调用受控工具处理。
```
