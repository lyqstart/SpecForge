...
/** 用户级 manifest 文件名 */
const USER_MANIFEST_FILENAME = `${SPEC_DIR_NAME.slice(1)}-manifest.json`
...
export async function readUserManifest(
  ...
  const manifestPath = join(userLevelDir, USER_MANIFEST_FILENAME)
...
export async function readAndValidateManifest(targetDir: string): Promise<ManifestResult> {
  const manifestPath = join(targetDir, USER_MANIFEST_FILENAME)
